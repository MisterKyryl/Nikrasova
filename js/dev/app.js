    ...
    let initSpollers2 = function(spollersArray2, matchMedia = false) {
      spollersArray2.forEach((spollersBlock) => {
        ...
        if (matchMedia.matches || !matchMedia) {
          ...
          if (spollersBlock.hasAttribute("data-fls-spollers-hover") && !document.documentElement.hasAttribute("data-fls-mobile")) {
            const spollerItems = spollersBlock.querySelectorAll(":scope > details");
            const spollerSpeed = spollersBlock.dataset.flsSpollersSpeed ? parseInt(spollersBlock.dataset.flsSpollersSpeed) : 500;
            const oneSpoller = spollersBlock.hasAttribute("data-fls-spollers-one");
            spollerItems.forEach((spollerItem) => {
              const spollerTitle = spollerItem.querySelector(":scope > summary");
              let hoverTimeout;
              let hoverState = false;
              spollerTitle.addEventListener("mouseenter", () => {
                hoverState = true;
                clearTimeout(hoverTimeout);
                if (!spollerItem.open && !spollersBlock.querySelectorAll(".--slide").length) {
                  if (oneSpoller) hideSpollersBody2(spollersBlock);
                  spollerTitle.classList.add("--spoller-active");
                  spollerItem.open = true;
                  slideDown(spollerTitle.nextElementSibling, spollerSpeed);
                  setTimeout(() => {
                    if (!hoverState) {
                      spollerTitle.classList.remove("--spoller-active");
                      slideUp(spollerTitle.nextElementSibling, spollerSpeed);
                      setTimeout(() => spollerItem.open = false, spollerSpeed);
                    }
                  }, spollerSpeed + 10);
                }
              });
...
function initSliders() {
  if (document.querySelector(".slider-team")) {
    new Swiper(".slider-team", {
      ...
      */
      ...
      on: {
        beforeInit(swiper) {
          const wrapper = swiper.el.querySelector(".slider-team__wrapper");
          if (!wrapper) return;
          if (wrapper.dataset.duplicated) return;
          const slides = Array.from(wrapper.querySelectorAll(".slider-team__slide"));
          const duplicates = slides.map((slide2) => slide2.cloneNode(true));
          duplicates.forEach((dup) => wrapper.appendChild(dup));
          wrapper.dataset.duplicated = "true";
        },
        init(swiper) {
          updateSideClasses(swiper);
        },
        slideChange(swiper) {
          updateSideClasses(swiper);
        }
  ...
  if (document.querySelector(".slider-results")) {
    new Swiper(".slider-results", {
      ...
      // Кнопки "вліво/вправо"
      navigation: {
        prevEl: ".slider-results__button-prev",
        nextEl: ".slider-results__button-next"
      },
      // Брейкпоінти
      breakpoints: {
        360: {
...
function updateSideClasses(swiper) {
  const slides = swiper.slides;
  const activeIndex = swiper.activeIndex;
  slides.forEach((slide2, i) => {
    slide2.classList.remove("swiper-slide-left", "swiper-slide-right");
    slide2.classList.contains("swiper-slide-prev");
    slide2.classList.contains("swiper-slide-next");
    if (i < activeIndex - 1) {
      slide2.classList.add("swiper-slide-left");
    } else if (i > activeIndex + 1) {
      slide2.classList.add("swiper-slide-right");
    }
  });
}
